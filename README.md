...
### File Generator
**Description:**  
The first task requires generating a large file with single double value per line.

**Challenge:**  
Generating and writing to file one value at a time is ineffective and generating the whole 1gb file in memory requries high usage of RAM.
...
Space: O(k * m), where k - is the number of threads, m - size of buffer. Application uses static buffers, however each thread requires one.
...
**Challenge:**  
Application cannot load the whole file and sort it in memory and must finish execution in less than 30 minutes.
...
First, program iterates over all the values to split the file (O(n)), then it merges every two files by iterating over double stored in them (O(n)). Program repeats last action until all the files are merged (log(k)).  
  
Space: O(1), application only uses static buffers the size of which is set in advance.
...
Unfortunatly, C++ doesn't have its own implementation of large numbers, therefore, for arithmetic operations on numbers that may overflow double data type, application uses boost::multiprecision library. 

**Solution:**  
Application iterates over the file two times: first to calculate mean and standard deviation, second to calculate percentiles. In order to calcultate mean and standard deviation in parallel program uses [Welford's algorithm]([https://pages.github.com/](https://en.wikipedia.org/wiki/Algorithms_for_calculating_variance#:~:text=to%20a%20degree.-,Welford%27s%20online%20algorithm,-%5Bedit%5D)).
This algorithm allows calculating meand and standard deviation at the same time incrementally.

**Complexity:**  
Time: O(n), where n - is the number of doubles in file. Program only parses file two times.  
Space: O(1), program only uses static buffers.

# How to build
## Prerequisites
**OS:**  Windows.  Assumingly, the solution may also work on Linux, since it does not use any Win specific API, however it has not been tested.  
**Tools:**  Visual Studio 2022, the version used for development is 17.13.6. The solution uses C++20 standard.  
**Libraries:**  Boost::multiprecision library. The version of the boost library that was used for development is 1.85.0.
